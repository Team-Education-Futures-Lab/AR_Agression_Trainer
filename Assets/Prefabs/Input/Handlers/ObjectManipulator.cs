    ...
    public class ObjectManipulator : MonoBehaviour, IMixedRealityPointerHandler, IMixedRealityFocusChangedHandler, IMixedRealitySourcePoseHandler
    {
        ...
        private void ReleaseRigidBody(Vector3 velocity, Vector3 angularVelocity)
        {
            if (rigidBody != null)
            {
                ...
                {
                    if ((releaseBehavior & ObjectManipulator.ReleaseBehaviorType.KeepVelocity) != 0)
                    {
                        GetComponent<Rigidbody>().velocity = GetPointersVelocity();
                    }

                    if ((releaseBehavior & ObjectManipulator.ReleaseBehaviorType.KeepAngularVelocity) != 0)
                    {
                        GetComponent<Rigidbody>().angularVelocity = GetPointersAngularVelocity();
                    }
                }
            }
        }
